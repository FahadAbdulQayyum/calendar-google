...
// console.log('(savedEvents[savedEvents.length-1]?.wk === w[i] && savedEvents[savedEvents.length-1]?.hr === t[ii])',savedEvents[savedEvents.length-1]?.wk === w[ii] && savedEvents[savedEvents.length-1]?.hr === t[ii])
// console.log(savedEvents[savedEvents.length]?.wk + ' === ' + w[ii]+' ||| ' + savedEvents[savedEvents.length]?.hr +'==='+ t[i])
// console.log(savedEvents[savedEvents.length-1])
// console.log(savedEvents[i<savedEvents.length && i])
// console.log('(savedEvents[savedEvents.length-1]?.wk === w[i] && savedEvents[savedEvents.length-1]?.hr === t[ii])',(savedEvents[savedEvents.length-1]?.wk === w[ii] && savedEvents[savedEvents.length-1]?.hr === t[i]))
                                      ...
                                      i==0 
                                       ...
                                       <td key={ii} className={`box${t[ii]} box border p-5 text-center w-[8.4em] hover:bg-gray-100 hover:scale-110 transition-transform cursor-pointer`}>{t[ii]}</td>
                                        ...
                                        (savedEvents[i<=savedEvents.length && i]?.wk === w[i] && savedEvents[i<=savedEvents.length && i]?.hr === t[ii])
                                        ?
